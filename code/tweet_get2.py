...
def tweet_get_to_csv(word, wordcount):
    ...
    #取得するツイート数をここに記入
    for tweet in getter.collect(total = wordcount):
        # URLを削除
        clean_tweet = re.sub("https://[\w!\?/\+\-_~=;\.,\*&@#\$%\(\)'\[\]]+", "", tweet['text'])
        # @ツイートの宛先を削除
        clean_tweet = re.sub("@[\\w]{1,15}", "", clean_tweet)
        cnt += 1
